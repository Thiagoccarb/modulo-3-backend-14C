...
function validationBook(req, res, next) {
    ...
    if (!valid) {
        return res.status(statusCodes_1.default.BAD_REQUEST).json({ message: `O campo ${property} é obrigatório.` });
    }
    [valid, property] = validateValues(book);
    if (!valid) {
        return res.status(statusCodes_1.default.BAD_REQUEST).json({ message: `O campo ${property} não pode ser nulo ou vazio.` });
    }
    next();
}
;
